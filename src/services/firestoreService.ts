...
export class FirestoreService {
  ...
  static async getSystemSettings(): Promise<SystemSettings | null> {
    try {
      ...
      if (docSnap.exists()) {
        const data = docSnap.data();
        console.log('✅ Firebase: System settings retrieved');
        return {
          ...data,
          updatedAt: data.updatedAt?.toDate() || new Date()
        } as SystemSettings;
      } else {
        console.log('⚠️ Firebase: System settings not found, creating default settings...');
        ...
        console.log('✅ Firebase: Default system settings created');
      ...
      console.log('⚠️ Firebase: Returning default settings due to error');
      return {
        maintenanceMode: false,
        maintenanceMessage: 'System is under maintenance. Please try again later.',
        minWithdrawal: 100,
